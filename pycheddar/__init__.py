...
class clean_parser(object):
    def __init__(self, content):
        self.stream = ElementTree.iterparse(StringIO(content), events=("start", "end"))
        self.backup = None
        self.element = None
    ...
    def has_children(self):
        self.backup = next(self.stream)
        return self.backup[0] == "start"
...
class CheddarObject(object):
    ...
    @classmethod
    def from_xml(cls, stream, **kwargs):
        ...
        new._load_data_from_stream(stream, clean)

        # done -- return the new object
        return new


    def _load_data_from_stream(self, stream, clean = True):
        ...
        self._id = stream.element.get('id')
        self._code = stream.element.get('code')
            ...
            if stream.has_children():
                ...
                if (element_tag, child.tag) in singles:
                    ...
                    if hasattr(sys.modules[__name__], class_name):
                        ...
                        setattr(self, single_xml.tag, klass.from_xml(stream, parent = self))
                ...
                for indiv_xml in stream.get_children(child.tag):
                    ...
                    try:
                        ...
                        getattr(self, child.tag).append(klass.from_xml(stream, parent = self))
...
class Plan(CheddarObject):
    ...
    def all(cls):
        ...
        try: 
            plans = []
            stream = CheddarGetter.request('/plans/get/')

            # make a list of Plan objects and return it
            for plan_xml in stream.get_tags('plan'):
    ...
    def get(cls, code):
        ...
        stream = CheddarGetter.request('/plans/get/', code = code)
...
class Customer(CheddarObject):
    ...
    def search(cls, **kwargs):
        ...
        try:
            customers = []
            stream = CheddarGetter.request('/customers/get/', **kwargs)
            for customer_xml in stream.get_tags('customer'):
    ...
    def get(cls, code):
        ...
        #else:
        stream = CheddarGetter.request('/customers/get/', code=code)
    ...
    def save(self):
        ...
        if self.is_new():
            ...
            stream = CheddarGetter.request('/customers/new/', product_code = self._product_code, code = self._code, **kwargs)
        else:
            # okay, this isn't new -- send the update request
            stream = CheddarGetter.request('/customers/edit/', product_code = self._product_code, code = self._code, **kwargs)
        ...
        #from django.conf import settings
        for customer_xml in stream.get_tags('customer'):
            self._load_data_from_stream(customer_xml)
            break
    ...
    def delete(self):
        ...
        stream = CheddarGetter.request('/customers/delete/', product_code = self._product_code, code = self._code)
    ...
    def add_charge(self, charge_code, item_code, amount = 0.0, quantity = 1, description = None):
        ...
        stream = CheddarGetter.request('/customers/add-charge/', product_code = self._product_code, code = self.code, **kwargs)
...
class Subscription(CheddarObject):
    ...
    def save(self):
        ...
        stream = CheddarGetter.request('/customers/edit-subscription/', product_code = self._product_code, code = self.customer.code, **kwargs)

        # either way, I should get a well-formed customer XML response
        # that can now be loaded into this object
        for subscription_xml in stream.get_tags("subscription"):
            self._load_data_from_stream(subscription_xml)
            break
    ...
    def delete(self):
        ...
        stream = CheddarGetter.request('/customers/cancel/', product_code = self._product_code, code = self.customer.code, **kwargs)
...
class Item(CheddarObject):
    ...
    def save(self):
        ...
        self._load_data_from_stream(stream)
        return self
